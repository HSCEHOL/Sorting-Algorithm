...
1048576개의 데이터들이 주어졌는데 , 이것이 정렬되거나 역정렬되서 주어지는 경우가 있을까? 실제로 데이터의 개수가 100000개만 넘어가도 데이터가 정렬되거나 역정렬 되어있는 경우는 거의 없다고 한다. 그렇다면 랜덤 데이터가 주어졌을때 , 수행 속도가 가장 빠른 알고리즘이 결국 제일 효율적이고 널리 쓰이는 알고리즘이 아닐까? 그래서 한번 랜덤 데이터가 주어진 상황에서 5개의 알고리즘의 속도를 한번 비교해보고자 그래프를 나타내보았다. 1048576의 버블정렬 값은 위에 말한듯이 컴파일러가 중간에 계속 중단되어 정확한 값을 구하지 못했지만 그래프를 위하여 근사한 추정치를 넣어놓았다.
...
이번엔 시간 복잡도를 통해 알아보자.<br>

![시간복잡도](https://user-images.githubusercontent.com/101388379/166881366-2c17786f-ce97-4e18-b46d-5e28026066a6.PNG)


위 모든 자료를 종합해보면 퀵 정렬이 정렬 알고리즘 중에서는 가장 효율적이라는 것을 보여주고 있다. <br>
물론 정렬된 데이터 , 역정렬된 데이터가 주어진 경우에는 퀵 정렬은 효율적이지 못하다.<br>
하지만 위에서도 말했듯이 십만개 , 백만개의 데이터가 정렬되어 주어지는 경우는 거의 없다는 점을 생각해보면 <br>
결국 랜덤 데이터가 주어졌을때의 경우만을 생각하게 되어 퀵정렬이 가장 효율적인 정렬 알고리즘이라는 결론이 나온다.


---
## 마치며

이 정렬 알고리즘의 수행 시간을 CPU가 다른 두개의 노트북과 컴퓨터로 분석했는데 , 당연한 얘기지만 좋은 CPU를 가진 컴퓨터가 확연히 수행 시간이 빠르게 나왔다.<br>
CPU의 차이를 체감할 일이 별로 없었는데 , 눈으로 확인이 되니 신기했다. 또한 데이터 입력 수를 32 ~ 1048576 까지 조정하며 광범위한 범위에서 수행 속도를 측정했는데 ,
그래프를 보면 알 수 있듯이 , 범위가 너무 광범위하여 데이터 입력 수를 작게할때는 그래프 상으로 차이가 거의 확인되지 않는다. 좀 범위를 작게 설정하여 , 0.001초와 같이 미세한 차이도 그래프 상에서 확인이 될 수 있게 하면 좋을 것 같다. 마무리로 한눈에 정렬 알고리즘의 여러 종류들이 어떻게 작동하는지에 대해 알아볼 수 있는 간단한 gif 형식으로 나타낸 사진으로 끝을 내겠다.

![KakaoTalk_20220505_035317893](https://user-images.githubusercontent.com/101388379/166956198-6bc98725-399d-4f13-83cc-d4d17f1281d3.gif)
